...
function connectPI() {
  ...
  websocket.onmessage = (evt) => {
    ...
    if (event === 'didReceiveSettings' && payload && payload.settings) {
      loadSettingsIntoUI(payload.settings);
    } else if (event === 'didReceiveGlobalSettings' && payload && payload.settings) {
...
function loadSettingsIntoUI(s) {
  ...
  if (Array.isArray(s.group_device_ids)) {
    setTimeout(() => {
      for (const opt of groupDevices.options) opt.selected = s.group_device_ids.includes(opt.value);
    }, 250);
  }
...
loadDevicesBtn.addEventListener('click', async () => {
  ...
  } catch (e) { alert('Failed to load devices: ' + e.message); }
});
...
loadScenesBtn.addEventListener('click', async () => {
  ...
  } catch (e) { alert('Failed to load scenes: ' + e.message); }
});
